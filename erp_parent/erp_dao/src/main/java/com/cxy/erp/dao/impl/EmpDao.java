...
import com.cxy.erp.entity.News;
/**
 * 员工数据访问类
...
public class EmpDao extends BaseDao<Emp> implements IEmpDao {
	...
	@Override
	public Long getStateCount(Long uuid) {
		// 获取当前线程
		// Session session =
		 ...
		 * "{call emp_getState(?,?)}"); call.setLong(1, uuid);
		 * call.registerOutParameter(2, OracleType.STYLE_LONG); call.execute();
		 * System.out.println(call.getObject(2));
		 */

		/*
		 * List<Long> list = sqlQuery.list(); if (list.size() > 0) { return
		 * list.get(0); }
		 */
		List<Long> list = (List<Long>) this.getHibernateTemplate()
				.find("select count(1) From News where operer=? and state=?", uuid, News.STATE_NO_READ);
		if (list.size() > 0) {
			return list.get(0);
		}
